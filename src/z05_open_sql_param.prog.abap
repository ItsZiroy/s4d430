...
TYPES ty_t_customers TYPE STANDARD TABLE OF z05_customer_with_parameter
...
************************************************************************
  ...
  SELECT * FROM z05_customer_with_parameter( p_flights_before = @sy-datum )
            WHERE city = @pa_city
            ORDER BY city, name
            INTO TABLE @gt_customers.
  " Identisch zu
  SELECT * FROM z05_customer_with_parameter
            WHERE city = @pa_city
            ORDER BY city, name
            INTO TABLE @gt_customers.
